...
ggsave(filename="lasso_logistic/KEGG/plaque_logisticlasso_raw.svg", AUC_plot_1,
       width=5, height=4)
...
ggsave(filename="lasso_logistic/KEGG/plaque_biomarker_boxplot.svg", marker_boxplot,
       width=4, height=4)
...
roc_curve <- ggplot(roc_df, aes(x = 1 - Specificity, y = Sensitivity)) +
  ...
  labs(
    title = sprintf("Plaque KEGG (AUC %.3f)", auc_value),
