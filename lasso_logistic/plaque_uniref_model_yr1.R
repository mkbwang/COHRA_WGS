...
plaque_counts <- read.table("counts_cleaning/strata/plaque_uniref_counts_yr1.tsv",
...
# load DAA results
DAA_uniref_results <- read.table("DAA/yr1/plaque/plaque_DA_uniref_table.tsv",
                                 sep='\t', header=1)
...
plaque_counts <- plaque_counts_imputed[, marker_uniref]
...
frequency_cutoffs <- c(0.1, 0.15, 0.2, 0.25, 0.3)
...
for (j in 1:5){
  ...
  counts_positive_features <- rowSums(plaque_counts_imputed[, positive_features])
  counts_negative_features <- rowSums(plaque_counts_imputed[, negative_features])
...
saveRDS(predictive_features,
        "lasso_logistic/uniref/plaque_predictive_features_yr1.rds")
